...
const setRangeText$p = rpl => s => e => mode => area => {
  const $5 = (() => {
    if (mode.tag === "Preserve") { return "preserve"; }
    if (mode.tag === "Select") { return "select"; }
    if (mode.tag === "Start") { return "start"; }
    if (mode.tag === "End") { return "end"; }
    $runtime.fail();
  })();
  return () => _setRangeText(rpl, s, e, $5, area);
};
const list = x => {
  const $1 = _list(x);
  return () => {
